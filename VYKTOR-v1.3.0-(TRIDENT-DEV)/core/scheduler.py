...
def evolve(task_name, seed_code, rounds=30, pop_size=12, timeout=0.3, max_workers=None, cfg=None, session_id=None):
    ...
    if pool:
        ...
        log_event({"task":task_name,"event":"gen_tick","gen":0,"score":best.score,"ok":best.ok,"runtime":best.runtime,"origin":best.origin,"ok_rate":ok_rate,"avg_runtime":avg_runtime})
    ...
    for r in range(1, rounds):
        ...
        log_event({"task":task_name,"event":"gen_tick","gen":r,"score":(top.score if top else 0.0),"ok":(top.ok if top else False),"runtime":(top.runtime if top else 0.0),"origin":(top.origin if top else "seed"),"ok_rate":ok_rate,"avg_runtime":avg_runtime,"bandit":{"weights":op_weights}})
