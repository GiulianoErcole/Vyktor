...
class VyktorMutator(ast.NodeTransformer):
    ...
    def visit_For(self, node):
        ...
        if random.random() < 0.10 and isinstance(node.iter, ast.Call) and getattr(node.iter.func, 'id', '') == 'range':
            args = node.iter.args
            if len(args) == 1: node.iter.args = [ast.Constant(0), args[0], ast.Constant(2)]
            elif len(args) == 2: node.iter.args = [args[0], args[1], ast.Constant(2)]
        return node
def mutate_ast(code: str) -> str:
    try:
        tree = ast.parse(code); VyktorMutator().visit(tree); ast.fix_missing_locations(tree)
