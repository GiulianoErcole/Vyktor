...
class Candidate:
    def __init__(self, code: str, origin: str):
        ...
        self.score = 0.0; self.ok = False; self.runtime = 0.0; self.metrics = {}
    def evaluate(self, timeout=1.0):
        ok, out, rt = sandbox_exec(self.code, timeout)
        self.ok = ok; self.runtime = rt; self.metrics = {'ok': 1.0 if ok else 0.0, 'runtime': rt, 'length': len(self.code)}
