...
import websockets
LOG_PATH = "results/live_feed.log"; DB_PATH = "results/telemetry.sqlite"; connected = set()
...
async def tail_log():
    ...
    while True:
        ...
        except Exception as e: print("Log tail error:", e)
...
class StaticServer(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/summary":
            ...
            except Exception: self.send_response(404); self.end_headers(); self.wfile.write(b"{}"); return
        if self.path == "/api/history":
            try:
                conn=sqlite3.connect(DB_PATH); cur=conn.execute("SELECT id, ts_start, ts_end, meta FROM sessions ORDER BY id DESC LIMIT 5")
                rows=[{"id":r[0],"ts_start":r[1],"ts_end":r[2],"meta":r[3]} for r in cur.fetchall()]
                self.send_response(200); self.send_header("Content-Type","application/json"); self.end_headers(); self.wfile.write(json.dumps(rows).encode("utf-8")); return
            except Exception: self.send_response(500); self.end_headers(); self.wfile.write(b"[]"); return
        return super().do_GET()
def http_server():
